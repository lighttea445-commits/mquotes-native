...
/** Write back the cached quote after fetching a new one in WIDGET_UPDATE. */
...
async function widgetTaskHandler(props: WidgetTaskHandlerProps) {
  ...
  if (
    ...
    const config = (await loadWidgetConfig(widgetInfo.widgetId)) ?? defaultConfig();
    ...
    if (!quote || widgetAction === 'WIDGET_UPDATE') {
      ...
      if (fetched) {
        ...
        await persistCachedQuote(widgetInfo.widgetId, quote);
      }
    }
